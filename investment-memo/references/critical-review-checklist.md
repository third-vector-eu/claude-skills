...
Go through each check below. For every hit, capture:

- The exact quote from the memo.
...
Read every sentence. Flag any claim that lacks a name, a number, or a mechanism.

Examples of fluff:
- "We believe customers will pay for this." (Which customers? How much?)
- "AI is transforming customer support." (No one cares; what specifically changes?)
- "We have a best-in-class team." (Credentials-only; no actual edge.)
- "There's a huge market opportunity." (How big, measured how?)

Rule: if removing the sentence doesn't change what the reader knows, remove the sentence.

## 2. Contradiction scan

Look for inconsistencies between sections. Common ones:

- ICP says "mid-market" but pricing implies SMB.
- GTM says "self-serve" but ACV is $80k.
- Wedge is described as small but the 90-day plan implies massive scope.
- "Why now" cites a trend that "market size" data contradicts.
- Competitor section dismisses incumbents as slow, but the GTM section relies on them being fast enough to validate the market.

Flag every one, with both quotes side by side.

## 3. Wedge sharpness

- Can the wedge be described in one sentence? If it needs two, it's too broad.
- Is it shippable in a quarter, or is it secretly the whole vision?
- Does the wedge have a first user — someone who, on day one, gets value? Or is it a platform waiting for a use case?

If the wedge is soft, the whole memo is soft. Press hardest here.

## 4. Why-now test

Read the "Why Now" section and ask: could this paragraph have been written in 2022? 2020? 2015?

If yes to any of those, it's not a why-now — it's an evergreen truth. Push for what specifically changed in the last 18-36 months.

Valid why-nows cite: tech capability shifts (crossed a threshold), regulatory moves (specific rules), behavioural changes (post-COVID, AI budget creation), or incumbent disruption (named companies losing ground).
...
- Is the first-customer motion concrete, or does it rely on "content marketing" / "inbound" / "thought leadership"?
- For the first 10 customers: are they named, or is it "my network"?
...
- Does the pricing imply a business model that matches the GTM motion?
- Is the pricing anchored to real comparables, cost structure, or willingness-to-pay data?
- Is the pricing tagged as hypothesis (it almost always should be, pre-revenue)?
- Is there at least one mention of gross margin logic?

## 9. Market-size sanity

- Is TAM calculated bottoms-up (count accounts × ACV) or top-down (referencing a research report)?
- Are the assumptions behind the math stated explicitly?
...
For each major bet in the memo, ask: **what would prove this wrong?**

If the answer is "nothing would", that's not a belief — it's a creed. Flag it.

Typical load-bearing bets to pressure-test:

- Customers will pay for this.
- The technical solution will work.
- The GTM motion will scale.
- The market will grow.
- The team can execute this.

For each: is there a stated falsification criterion and a test plan? If not, request one.

## 11. Distribution of attention
...
## 14. The "smart skeptic" test

Read the memo from the perspective of a smart, friendly skeptic. Ask:
